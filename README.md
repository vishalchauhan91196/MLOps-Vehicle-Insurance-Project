...
This project demonstrates a production-ready end-to-end MLOps pipeline for vehicle insurance data.
It showcases how machine learning systems are built, versioned, deployed, and automated in real-world environments — going far beyond experimentation and notebooks.

The project is designed to highlight industry-grade MLOps practices, cloud integration, and CI/CD automation.

## ✨ Key Highlights

- End-to-end ML pipeline (ingestion → deployment)
...
- The `pyproject.toml` file reference `requirements.txt` so that package dependencies are automatically pulled from there. 
...
4. Copy the Python MongoDB connection string.
...
- Create an S3 bucket.
- Develop code to push/pull models to/from the S3 bucket in `src.cloud_storage` and `entity/s3_estimator.py`.
...
## ⭐ This project demonstrates how real-world ML systems are engineered, deployed, and automated at scale.