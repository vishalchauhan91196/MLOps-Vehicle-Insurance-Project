...
class VehicleData:
    def __init__(self,
                ...
                Vehicle_Damage_Yes):
        """  Vehicle Data constructor. Inputs all features of the trained model for prediction. """
        try:
            self.Gender = Gender
            self.Age = Age
...
class VehicleDataClassifier:
    ...
    def predict(self, dataframe) -> str:
        """  This method returns Prediction in string format. """
        try:
            logging.info("Entered predict method of VehicleDataClassifier class")
            model = VehicleDataEstimator(
                bucket_name=self.prediction_pipeline_config.model_bucket_name,
                model_path=self.prediction_pipeline_config.model_file_path,
            )
            result =  model.predict(dataframe)
            logging.info("Exiting predict method of VehicleDataClassifier class")
